...
# Ensure that three arguments are provided
if [ $# -ne 2 ]; then
...
for i in $(seq 1 $PAGE_COUNT); do
 PAGE_PDF=$(printf "$TEMP_DIR/page_%04d.pdf" "$i")
 ...
 # generate pdf
 convert -size ${PDF_WIDTH}x${PDF_HEIGHT} xc:transparent "$TEMP_DIR/bitmap_page_$i.png" -geometry +${RAND_X}+${RAND_Y} -composite "$TEMP_DIR/overlay_$i.pdf"
...
echo "Stamped PDF created: $OUTPUT_PDF"
